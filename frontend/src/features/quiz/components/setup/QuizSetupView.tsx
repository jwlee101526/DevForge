...
export const QuizSetupView: React.FC<QuizSetupViewProps> = ({
  ...
  return (
    <div className="animate-fadeIn space-y-5">
      <div>
        <h2 className="text-2xl font-black tracking-normal text-slate-950">개발 지식 퀴즈 설정</h2>
            ...
            <CardContent className="space-y-5 p-5">
              <div className="flex flex-col gap-2 pb-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="text-left sm:text-right">
                  <span className="block text-xs font-bold text-slate-500">선택된 항목 수</span>
                  <strong className="text-2xl font-black text-[#0f766e]">
                    {selectedItemCount.toLocaleString()}개
                  </strong>
                  <span className="ml-1 text-sm font-bold text-slate-500">/ {totalItems.toLocaleString()}개</span>
                </div>
              </div>
